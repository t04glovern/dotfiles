...
VOLUMES_LIST=~/.scripts/aws_snapshot/volumes-list
SNAPSHOT_INFO=~/.scripts/aws_snapshot/snapshot_info
DATE=`date +%Y-%m-%d`
...
SNAP_CREATION=~/.scripts/aws_snapshot/snap_creation
SNAP_DELETION=~/.scripts/aws_snapshot/snap_deletion

EMAIL_LIST=nathan@glovers.id.au
...
cat $SNAP_CREATION $SNAP_DELETION > ~/.scripts/aws_snapshot/mail_report

# Sending the mail Update

cat ~/.scripts/aws_snapshot/mail_report | mail -s "Volume Snapshots Status" $EMAIL_LIST
